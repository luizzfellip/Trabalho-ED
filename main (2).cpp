...
#include <sstream>
#include <stdexcept>
#include <chrono> // so para ver o tempo de execucao do programaa
using namespace std;
...
bool procura_maior(ifstream arqF[],ofstream arqS[],int rodada = 0)
{
	...
	bool comValor[numPartes];	// marca quais arquivos ainda tem dados, serve para chegar ate o NULL ou até o tamBloco
 	int QteArqComValor = 0;      // conta qnts arquivos tem dados
 	int blocoTamanho = 1;       // tamanhos dos blocos
    int lidos[numPartes] = {0}; // conta qnts registros foram lidos em cada bloco, manda até onde vai o comValor
    
    for(int i=0; i<rodada; i++){ // da o tamanho que vai aumentando com o tempo
        blocoTamanho *= numPartes;
    }
	
	int indiceSaida = 0;  // aonde sera a saisa
	int escritosNoArquivoAtual = 0; // quantos registros já foram gravados em s_i atual
	...
	// 3. escreve o maior na saida	
		arqS[indiceSaida].write(reinterpret_cast<char*>(&buffer[posMaior]),sizeof(Registro));
		escritosNoArquivoAtual++;
		
		// troca de arquivo se atingiu o tamanho do blocoTamanho
		if(escritosNoArquivoAtual == blocoTamanho){
		    escritosNoArquivoAtual = 0; // reseta para o proximo
		    indiceSaida++;
		}
		
		// verifica se pode passar para o proximo do registro, se não nao pode continuar
		if((lidos[posMaior] < blocoTamanho) && (arqF[posMaior].read(reinterpret_cast<char*>(&buffer[posMaior]),sizeof(Registro)))){
			lidos[posMaior]++;
		}else{
    ...
    string nomeFinal = (rodada % 2 == 0) ? nomeBaseAqrS + "0.bin" : nomeBaseAqrF + "0.bin";	
    ifstream testeFinal(nomeFinal, ios::binary | ios::ate);
    long long tamFinal = testeFinal.tellg();
    testeFinal.close();

	
	ifstream arqInfo("Tamanho_total_bytes.txt");
	long long tamanhoEsperado;
	arqInfo >> tamanhoEsperado;
	arqInfo.close();

   // se o arquivo 0 contém todos os registros retorna true e acaba
    return(tamFinal == tamanhoEsperado);
}
